...
exports.setQualityAppraisalsValues = async (req, res) => {
  ...
  HSEArticleModelClass.findById(articleId, async (err, article) => {
    ...
    if (
      !(
        user._id.equals(article._qualityAppraisalsJunior)
        || user._id.equals(article._qualityAppraisalsSenior)
      )
    ) {
      return res.status(404).send({
  ...
  HSEArticleModelClass.findById(articleId, async (err, article) => {
    ...
    if (
      !(
        ...
        || article._qualityAppraiasalsSenior.equals(user._id)
      )
    ) {
      return res.status(404).send({
...
exports.setFullQualityAppraisalCompleteOrResolve = async (req, res) => {
  ...
  HSEArticleModelClass.findById(articleId, async (err, article) => {
    ...
    if (article.qualityAppraisalsJuniorCompleted && article.qualityAppraisalsSeniorCompleted) {
      ...
      await HSEArticleQualityAppraisalModelClass.findById(
        article.qualityAppraisalsJuniorInput,
        async (err, qualityAppraisalJunior) => {
