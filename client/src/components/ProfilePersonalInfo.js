...
const ProfilePersonalInfo = ({ t }) => {
  ...
  const toggleRole = (e) => {
    // check the target name and add/remove
    // from the roles array depending on if it's
    // checked or not
  };
  ...
  const Edit = () => {
    return (
      <div>
        ...
        <div class="form-group update-role">
  ...
  const save = (e) => {
    e.preventDefault();
    // Call the ProfileService.update function
    // passing in the latest deets
    const info = {
      firstName,
      lastName,
      language,
